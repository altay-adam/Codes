	...
	class Program
	{
		static void Main(string[] args)
		{
			...
			while (true)
			{
				...
				int counter = 0;
				while (temp >= 1)
				{  //In this loop, we're gonna find every divisors that can divide the triangle numbers sequence.
					if (temp % number == 0)
					{   //Here, we divide temp to the number until our remainder equals 0. This is because to find multipliers of the temp(Value).
						temp = temp / number;
						counter++;
					}
					else
					{
						...
						if (counter > 0)
						{
							product *= (counter + 1);  //Product equals that "(counter+1)*product". This is because to find multipliers of the temp(Value).
						}
						counter = 0;
						if (temp == 1)
						{
